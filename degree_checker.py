...
EXCEL_URL = (
    "https://riskcontrolacademy-my.sharepoint.com/:x:/g/personal/investigation_riskcontrolnigeria_com/Eaa7o9lJqc9Ojcl7sKWFGokB84lrGwTXKerqQcobShCM6A?e=N809qQ&download=1"
)
...
required_columns = {"NAME", "CLIENT", "DATE RECEIVED"}
...
PASSWORD = os.environ.get("USER_PASSWORD")
TARGET = "abbajanet1@gmail.com"  # Change this if you want dynamic recipient
...
def send_reminder(row, days_left):
    name = row["NAME"]
    client = row.get("CLIENT", "Unspecified")
    received = row["DATE RECEIVED"].strftime("%d %b %Y")
    due = row["TAT DUE"].strftime("%d %b %Y")
...
# 4. Iterate and send alerts --------------------------------------------------
for _, r in df.iterrows():
    if pd.isna(r["TAT DUE"]):
        continue
    days_remaining = (r["TAT DUE"] - today).days
    if days_remaining in {76, 79, 30, 14, 1, 0, 2}:
        send_reminder(r, days_remaining)
